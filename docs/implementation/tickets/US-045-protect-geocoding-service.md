...
Title: US-045 - Protect Geocoding Service
...
As a JoeKnock system administrator, I want geocoding requests controlled through the JoeKnock backend, so that the application does not expose or abuse the external geocoding service.

# 3. Objective

Deliver the approved user-story outcome for Issue #45 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.

# 4. Scope

## 4.1 Included

- Frontend does not call geocoding providers directly.
- MVP geocoding flow is mediated through `POST /api/properties/resolve`.
- Backend performs reverse geocoding as an internal implementation detail.
- Requests require authentication.
- Backend validates coordinates input.
- Geocoding/provider behavior is handled behind JoeKnock API boundaries.
- OpenStreetMap attribution requirements are supported by the application.
