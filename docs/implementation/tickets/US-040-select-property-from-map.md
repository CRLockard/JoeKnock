...
Title: US-040 - Select Property from Map
...
As a sales representative, I want to select a property directly from the map, so that I can begin an interaction without navigating through multiple screens.

# 3. Objective

Deliver the approved user-story outcome for Issue #40 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.
