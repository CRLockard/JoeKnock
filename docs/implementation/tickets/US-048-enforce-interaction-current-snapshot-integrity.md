...
Title: US-048 Î“Ã‡Ã¶ Enforce Interaction Current-Snapshot Integrity
...
Implement and verify the cross-cutting invariant rules from ADR-008 and reporting semantics used by ADR-010.
...
- Latest/current selection is deterministic: `changed_at DESC`, then `id DESC` tie-break.
...
- Preserve immutable snapshot behavior and protect against duplicate side effects on retry.
