...
As a manager or administrator, I want to update a status, so that the organization's workflow can evolve.

# 3. Objective

Deliver the approved user-story outcome for Issue #16 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.
