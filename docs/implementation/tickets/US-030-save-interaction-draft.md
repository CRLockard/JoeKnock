...
Title: US-030 - Save Interaction Draft
...
Implementation Phase: Phase 4 - In-Map Capture and Resilience
...
As a sales representative, I want my partially completed interaction to be protected from accidental data loss, so that a dead battery, dropped connection, or accidental interruption does not force me to start over.

# 3. Objective

Deliver the approved user-story outcome for Issue #30 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.
...
- None until submission; local draft storage should be keyed to the selected property context

- Use existing documented constraints and relationships; do not introduce unapproved schema changes.

# 7. API Contract

- None; this story is local-client behavior until data is submitted.
