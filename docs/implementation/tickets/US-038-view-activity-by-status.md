...
Title: US-038 - View Activity by Status
...
Deliver the approved user-story outcome for Issue #38 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.
...
- Counts use interaction-group semantics for MVP knock logic.
- Snapshot revisions in existing interaction groups are not counted as additional knocks.
...
- Results respect selected date range.
