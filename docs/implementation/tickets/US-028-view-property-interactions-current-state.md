...
Title: US-028 - View Property Interactions (Current State)
...
As a sales representative, I want to see the current interaction state for a property, so that I know the latest authorized information before recording updates.

# 3. Objective

Deliver the approved user-story outcome for Issue #28 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.
...
- Multiple interaction groups can exist at one property across representatives.
- Current snapshots are identifiable.
- Results respect organization isolation and visibility permissions.
- Normal user-facing historical snapshot timeline is not exposed in MVP.
