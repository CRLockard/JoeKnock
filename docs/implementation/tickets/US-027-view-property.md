...
As a sales representative, I want to view property information after selecting it on the map, so that I can review its current information before recording an interaction.

# 3. Objective

Deliver the approved user-story outcome for Issue #27 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.

# 4. Scope

## 4.1 Included

- Selecting a property retrieves property details.
- Address information is displayed.
- Current interaction state for that property is retrievable for authorized users.
- User can access interaction workflow from the property view.
- Property and interaction data respect organization/visibility permissions.
