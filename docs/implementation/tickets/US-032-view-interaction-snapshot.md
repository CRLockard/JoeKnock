...
Title: US-032 - View Interaction Snapshot
...
Deliver the approved user-story outcome for Issue #32 with endpoint, authorization, and data behavior aligned to the finalized MVP architecture. The completed implementation should satisfy the acceptance criteria without expanding scope beyond this story.
...
- Snapshot displays `initial_interaction_at` (first interaction date for the interaction group, stable across revisions).
- Snapshot displays `changed_at` (timestamp for the current snapshot/revision state).
- Snapshot displays notes when present.
- Snapshot displays contact information when present.
- Snapshot identifies the representative who recorded the interaction group.
- Snapshot identifies the associated property.
- No `interaction_at` field is introduced or relied upon.
